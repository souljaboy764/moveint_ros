...
class RMDNHRINode:
	...
	def step(self):
		...
		_, self.alpha, _, _, _, robot_goals = self.model(self.readings[None], None)
		print(torch.nn.Softmax(-1)(self.alpha)[0])
		robot_right_goal = robot_goals[0, :3].cpu().numpy()
		robot_left_goal = robot_goals[0, 3:].cpu().numpy()
...
if __name__=='__main__':
	with torch.no_grad():
		...
		controller = RMDNHRINode('models/rmdvae_diagcov_expdistcontrastive_humanlhand_norel_trial0_380.pth')
