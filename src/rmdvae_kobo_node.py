...
class RMDVAEHRINode:
	def __init__(self, ckpt_path):
		...
		self.model = RMDVAE(input_dim,6,ckpt['args']).to(device)
	...
	def observe_human(self):
		...
		self.make_marker(human_right_transfom.transform.translation.x, human_right_transfom.transform.translation.y, human_right_transfom.transform.translation.z, 0, 0, 0, stamp)
		self.make_marker(human_left_transfom.transform.translation.x, human_left_transfom.transform.translation.y, human_left_transfom.transform.translation.z, 0, 0, 0, stamp)
...
if __name__=='__main__':
	with torch.no_grad():
		...
		controller = RMDVAEHRINode(os.path.join(pkgPath.get_path('rmdn_hri_ros'),'models/rmdvae_kobo_leftin_bihandover.pth'))
		...
		while not rospy.is_shutdown():
			...
			# if True:
				...
				if count > 150 and ((controller.readings[:6].cpu().numpy() - hand_pos)**2).sum() < 0.03:
					...
					controller.goal_pub.publish(controller.goal_msg)
					rospy.Rate(1).sleep()
					print('shutting down', count, ((controller.readings[:6].cpu().numpy() - hand_pos)**2).sum())
